...
def compute_my_own_descriptor(beat, winL, winR):
    ...
    [x_values[0], y_values[0]] = max(enumerate(beat[0:50]), key=operator.itemgetter(1))
    [x_values[1], y_values[1]] = min(enumerate(beat[75:90]), key=operator.itemgetter(1))
    [x_values[2], y_values[2]] = min(enumerate(beat[95:110]), key=operator.itemgetter(1))
    [x_values[3], y_values[3]] = max(enumerate(beat[150:199]), key=operator.itemgetter(1))
